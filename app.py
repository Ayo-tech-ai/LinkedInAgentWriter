...
<div class="custom-header">
    <h1 style="margin:0; color:white;">💼 AI/ML Multi-Platform Content Generator Pro</h1>
...
class VerifierAgent:
    ...
    def verify_factual_alignment(self, linkedin_post: str, search_results: str, threshold: int = 95) -> Dict:
        ...
        3. For each factual claim in the LinkedIn post (statistics, specific developments, outcomes, innovations mentioned):
           - Check if it is DIRECTLY supported by the research findings.
           - If partially supported or inferred, mark as PARTIAL.
           - If not supported or contradicted, mark as UNSUPPORTED.
        4. Calculate the factual alignment percentage:
           - FULLY_SUPPORTED claims count as 100%
           - PARTIALLY_SUPPORTED claims count as 50%
           - UNSUPPORTED claims count as 0%
        ...
        IMPORTANT: Be strict but fair. Claims about future potential or general benefits that aren't explicitly 
        stated in research should be marked as UNSUPPORTED.
        
        OUTPUT FORMAT (JSON):
        {{
            "alignment_percentage": 85.5,
            "status": "APPROVED" or "NEEDS_REVISION",
            "feedback": "Specific feedback on what claims need revision",
            "unsupported_claims": ["List specific unsupported claims here"],
            "partially_supported_claims": ["List partially supported claims here"],
            "supported_claims": ["List well-supported claims here"]
        }}
        ...
        try:
            response = self.llm.call(
                verification_prompt,
                "You are a meticulous fact-checker with expertise in AI and agriculture. You are objective, detail-oriented, and only approve claims with clear evidence."
            )
            ...
            if response:
                ...
                if json_match:
                    ...
                    if result.get("alignment_percentage", 0) >= threshold:
            ...
            return {
                ...
                "feedback": "Unable to complete verification. Please check content manually.",
            ...
            return {
                ...
                "unsupported_claims": ["Verification system error"],
                "partially_supported_claims": [],
                "supported_claims": []
            }
...
class EditorAgent:
    ...
    def edit_post(self, original_post: str, verification_feedback: Dict, search_results: str, target_tone: str) -> str:
        ...
        {chr(10).join(f'- {claim}' for claim in partially_supported)}
...
def execute_verification_loop(initial_linkedin_post: str, search_data: Dict, groq_llm, 
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        with st.spinner(f"🔍 Verifying factual alignment (Iteration {iteration})..."):
            verification_result = verifier.verify_factual_alignment(
                current_post, 
    ...
    st.markdown(f'<div class="verification-failed">⚠️ Max Iterations Reached: Best Available Alignment: {alignment_pct:.1f}%</div>', 
               unsafe_allow_html=True)
    st.warning(f"⚠️ Maximum iterations ({max_iterations}) reached. Using best available version.")
...
def execute_linkedin_workflow(query, groq_llm, max_results, serper_key, tone, threshold, max_iterations):
    ...
    with st.status("🔍 Verifying factual accuracy...", expanded=True) as status:
        ...
        if verification_history[-1].get("status") == "APPROVED":
            ...
            status.update(label=f"⚠️ Best effort: {final_alignment:.1f}% alignment after {iterations_used} iteration(s)", 
                         state="complete")
            ...
            try:
                ...
                if result is None:
                    ...
                    with tabs[0]:  # LinkedIn
                        ...
                        approval_status = result["verification_history"][-1].get("status", "UNKNOWN")
                        alignment_pct = result["verification_history"][-1].get("alignment_percentage", 0)
                        
                        if approval_status == "APPROVED":
                            st.markdown(f'<div class="ready-status">✅ APPROVED - {alignment_pct:.1f}% factual alignment</div>', 
                                       unsafe_allow_html=True)
                        else:
                            st.markdown(f'<div class="verification-pending">⚠️ BEST AVAILABLE - {alignment_pct:.1f}% factual alignment</div>', 
                                       unsafe_allow_html=True)
