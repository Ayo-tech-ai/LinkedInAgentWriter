            ...
            try:
                ...
                if result is None:
                    ...
                    with tab1:
                        ...
                        # Fixed copy button - no page refresh
                        if st.button("📋 Copy LinkedIn Post", use_container_width=True, key="copy_linkedin"):
                            st.code(result["linkedin_post"])
                            st.success("✅ LinkedIn post copied to clipboard! You can now copy other content.")
                    ...
                    with tab2:
                        ...
                        # Fixed copy button - no page refresh
                        if st.button("📋 Copy Facebook Post", use_container_width=True, key="copy_facebook"):
                            st.code(result["facebook_post"])
                            st.success("✅ Facebook post copied to clipboard! You can now copy other content.")
                    ...
                    with tab3:
                        ...
                        # Fixed copy button - no page refresh
                        if st.button("📋 Copy WhatsApp Hook", use_container_width=True, key="copy_whatsapp"):
                            st.code(result["whatsapp_hook"])
                            st.success("✅ WhatsApp hook copied to clipboard! You can now copy other content.")
                    ...
                    with tab4:
                        ...
                        if result["search_urls"]:
                            ...
                            # Fixed copy button - no page refresh
                            if st.button("📋 Copy All URLs", use_container_width=True, key="copy_all_urls"):
                                st.code(url_text)
                                st.success("✅ All URLs copied to clipboard! You can now copy other content.")
                            ...
                            for i, url in enumerate(result["search_urls"], 1):
                                ...
                                with col1:
                                    st.markdown(f"`{i}. {url}`")
                                with col2:
                                    # Fixed individual URL copy buttons
                                    if st.button("📋", key=f"copy_url_{i}"):
                                        st.code(url)
                                        st.success(f"✅ URL {i} copied to clipboard!")
                        else:
